...
export const useEmailAI = () => {
  ...
  const fetchEmailTemplates = useCallback(async (category?: string): Promise<EmailTemplate[]> => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to fetch email templates';
...
function createFallbackEmailAnalysis(subject: string, body: string): EmailAnalysis {
  ...
  if (wordCount > 300)
    improvements.push({ type: 'issue', description: 'Email is too long (over 300 words)' });
  if (avgSentenceLength > 25)
    improvements.push({ type: 'issue', description: 'Sentences are too long' });
  if (paragraphCount < 3)
    improvements.push({
      type: 'suggestion',
      description: 'Consider adding more paragraphs for readability',
    });
  if (!body.includes('?'))
    improvements.push({
      type: 'suggestion',
      description: 'Consider adding a question to encourage response',
    });
