...
const AIAssistantChat: React.FC<AIAssistantChatProps> = ({ assistantId: propAssistantId }) => {
  ...
  const [assistantInstructions, setAssistantInstructions] = useState(
    'You are a helpful sales assistant that helps with CRM tasks, deal analysis, and sales strategy.'
  );
  ...
  return (
    <div className="flex flex-col h-full rounded-xl border border-gray-200 overflow-hidden bg-white shadow-md transition-all duration-200 transform hover:shadow-lg">
      ...
      {showSettings && (
        <div className="p-6 bg-gradient-to-r from-indigo-50 via-blue-50 to-indigo-50 border-b border-indigo-100">
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-indigo-800 mb-2">Instructions</label>
              <textarea
            ...
            <div>
              <label className="block text-sm font-medium text-indigo-800 mb-2">Capabilities</label>
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setSelectedTools((prev) =>
                          prev.filter((tool) => tool !== 'code_interpreter')
                        );
                      }
                    }}
                    className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
      ...
      <div
        ...
        className="flex-1 p-4 overflow-y-auto bg-gray-50 bg-[linear-gradient(to_right,#f9fafb_1px,transparent_1px),linear-gradient(to_bottom,#f9fafb_1px,transparent_1px)] bg-[size:20px_20px]"
      >
            ...
            <div className="flex items-center space-x-2 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl shadow-sm border border-indigo-100">
              ...
              <div>
                ...
                <p className="text-xs text-indigo-600 opacity-75">
                  Using sophisticated AI to analyze your request...
                </p>
              </div>
            </div>
          )}
      ...
      <div className="bg-white p-4 border-t border-gray-200">
        ...
        <div className="flex justify-between items-center mt-3 px-1 text-xs text-gray-500">
          ...
          <div className="text-right">Powered by GPT-4o</div>
        </div>
      </div>
    </div>
  );
