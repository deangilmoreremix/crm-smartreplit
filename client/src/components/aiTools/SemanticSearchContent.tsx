...
const SemanticSearchContent: React.FC = () => {
  ...
  const [contactEmbeddings, setContactEmbeddings] = useState<
    { contactId: string; embedding: number[] }[]
  >([]);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchType === 'all' || searchType === 'contacts') {
        const contactsById = Object.values(contacts).reduce(
          (acc, contact) => {
      ...
      if (searchType === 'all' || searchType === 'deals') {
        const dealsById = Object.values(deals).reduce(
          (acc, deal) => {
            acc[deal.id] = deal;
            return acc;
          },
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 mt-1">
              Search your contacts, deals, and content using natural language and semantic
              understanding.
            </p>
          </div>
        </div>
        ...
        {isGeneratingEmbeddings ? (
          <div className="p-8 text-center bg-gray-50 rounded-lg">
            ...
            <p className="text-gray-500 text-sm mt-2">
              This may take a moment as we prepare the search database.
            </p>
          </div>
        ) : (
          ...
          <div className="mt-6">
            ...
            {searchResults.contacts.length === 0 && searchResults.deals.length === 0 && (
              <div className="p-8 text-center bg-gray-50 rounded-lg">
                ...
                <p className="text-gray-500 text-sm mt-2">
                  Try a different search query or expand your search criteria.
                </p>
              </div>
            )}
          </div>
