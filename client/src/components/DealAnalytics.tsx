...
const DealAnalytics: React.FC = () => {
    ...
    return (
      <div className="flex items-center mt-2">
        <div className="flex -space-x-2">
          ...
          {remainingCount > 0 && (
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold border-2 border-white dark:border-gray-900 ${
  ...
  const calculateKPIs = (): KPIMetric[] => {
    ...
    const representativeContacts = Object.values(contacts)
      .slice(0, 5)
      .map((contact) => ({
        id: contact.id,
        name: contact.name,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpiMetrics.map((metric, index) => (
          <div
            key={index}
            className={`relative overflow-hidden ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} backdrop-blur-xl border rounded-xl p-4`}
          >
            ...
            <div className="space-y-1">
              ...
              {metric.relatedContacts &&
                metric.relatedContacts.length > 0 &&
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div
          className={`${isDark ? 'bg-gradient-to-r from-blue-500/20 to-blue-600/20 border-blue-500/30' : 'bg-gradient-to-r from-blue-50 to-blue-100 border-blue-200'} border rounded-lg p-4`}
        >
          <div className="flex justify-between items-start">
            <div>
              ...
              <p
                className={`text-xl font-semibold mt-1 ${isDark ? 'text-white' : 'text-gray-900'}`}
              >
                ${Math.round(valuesByStatus.active / 1000)}k
              </p>
            </div>
          ...
          {valuesByStatus.activeDealsWithContacts &&
            valuesByStatus.activeDealsWithContacts.length > 0 &&
            renderAvatarStack(valuesByStatus.activeDealsWithContacts.map((deal) => deal.contact))}
        </div>

        <div
          className={`${isDark ? 'bg-gradient-to-r from-green-500/20 to-green-600/20 border-green-500/30' : 'bg-gradient-to-r from-green-50 to-green-100 border-green-200'} border rounded-lg p-4`}
        >
          <div className="flex justify-between items-start">
            <div>
              ...
              <p
                className={`text-xl font-semibold mt-1 ${isDark ? 'text-white' : 'text-gray-900'}`}
              >
                ${Math.round(valuesByStatus.won / 1000)}k
              </p>
            </div>
          ...
          {valuesByStatus.wonDealsWithContacts &&
            valuesByStatus.wonDealsWithContacts.length > 0 &&
            renderAvatarStack(valuesByStatus.wonDealsWithContacts.map((deal) => deal.contact))}
        </div>

        <div
          className={`${isDark ? 'bg-gradient-to-r from-red-500/20 to-red-600/20 border-red-500/30' : 'bg-gradient-to-r from-red-50 to-red-100 border-red-200'} border rounded-lg p-4`}
        >
          <div className="flex justify-between items-start">
            <div>
              <p className={`text-xs font-medium ${isDark ? 'text-red-300' : 'text-red-800'}`}>
                Lost Deals (YTD)
              </p>
              <p
                className={`text-xl font-semibold mt-1 ${isDark ? 'text-white' : 'text-gray-900'}`}
              >
                ${Math.round(valuesByStatus.lost / 1000)}k
              </p>
            </div>
          ...
          {valuesByStatus.lostDealsWithContacts &&
            valuesByStatus.lostDealsWithContacts.length > 0 &&
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div
          className={`${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} backdrop-blur-xl border rounded-xl shadow-sm`}
        >
          <div className="p-4 border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
            ...
            <div
              className={`text-xs px-2 py-1 rounded-full ${isDark ? 'bg-white/10 text-gray-300' : 'bg-gray-100 text-gray-600'}`}
            >
              {selectedPeriod} view
            </div>
          </div>
          ...
          <div className="p-4">
            <div className="space-y-4">
              {pipelineStageData.map((stage, index) => (
                <div
                  key={index}
                  className={`flex items-center justify-between p-3 ${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-lg`}
                >
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
              <div
                className={`border ${isDark ? 'border-indigo-500/30 bg-indigo-500/20' : 'border-indigo-100 bg-indigo-50'} rounded-lg p-2`}
              >
                <p className={`text-xs ${isDark ? 'text-indigo-300' : 'text-indigo-600'} mb-1`}>
                  Qual → Proposal
                </p>
                <p className={`text-lg font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {conversionRates.qualToProposal}%
                </p>
              </div>
              <div
                className={`border ${isDark ? 'border-indigo-500/30 bg-indigo-500/20' : 'border-indigo-100 bg-indigo-50'} rounded-lg p-2`}
