...
const SalesPerformanceDashboard: React.FC = () => {
  ...
  const generateAnalyticsData = () => {
    ...
    const enterpriseContacts = contactsArray.filter(
      (contact) =>
    ...
    const midMarketContacts = contactsArray.filter(
      (contact) =>
    ...
    return {
      ...
      topDeals: dealsArray.sort((a, b) => b.value - a.value).slice(0, 10),
                  ...
                  <p className="font-semibold text-green-600 text-lg">
                    {formatCurrency(
                      Object.values(contacts).reduce((total, contact) => {
                        if (contact.tags?.some((t) => t.toLowerCase().includes('enterprise')))
                          return total + 50000;
                        if (contact.tags?.some((t) => t.toLowerCase().includes('mid-market')))
                          return total + 25000;
                        return total + 10000;
                      }, 0)
                    )}
              ...
              >
                ...
                <CardContent>
                    ...
                    {Object.values(contacts)
                      ...
                      .map((contact, index) => {
                        const contactDeals = Object.values(deals).filter(
                          (deal) => deal.contactId === contact.id
                        );
                        const totalValue = contactDeals.reduce((sum, deal) => sum + deal.value, 0);
                        return (
                          <div
                            key={contact.id}
                            className="flex items-center space-x-3 p-3 rounded-lg bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="font-semibold text-green-600 text-sm">
                                {totalValue > 0
                                  ...
                                  : contact.tags?.some((t) =>
                                        t.toLowerCase().includes('enterprise')
                                      )
                                    ? '$50,000'
                                    : '$25,000'}
                              </p>
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-purple-600 text-lg">+12.5%</p>
                      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                        Growth Rate
                      </p>
                    </div>
