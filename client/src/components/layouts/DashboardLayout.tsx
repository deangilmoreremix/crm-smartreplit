...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({ children, className }) => {
  return (
    <div className={cn('flex items-center justify-between h-16 px-6', className)}>{children}</div>
  );
};
...
export const HeaderCenter: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="flex-1 flex items-center justify-center max-w-2xl mx-4">{children}</div>
);
