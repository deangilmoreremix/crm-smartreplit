  ...
  const loadQuickStats = async () => {
    try {
      ...
      const stats = {
        ...
        avgResponseTime:
          assistantsList.reduce((sum, a) => sum + a.performance.averageResponseTime, 0) /
          Math.max(assistantsList.length, 1),
      };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/assistants" className="text-sm text-indigo-600 hover:text-indigo-800">
          View All
        </Link>
      </div>
