  ...
  const handlePreview = () => {
    ...
    if (preview) {
      ...
      preview.document.write(
        `<!DOCTYPE html><html><head><title>Preview</title></head><body style="font-family:sans-serif;white-space:pre-wrap;padding:1rem;">${escaped}</body></html>`
      );
      preview.document.close();
    }
  };

  const csvData = result ? result.split('\n').map((line) => ({ line })) : [];
            ...
            <div className="flex justify-between items-center p-4 border-b border-indigo-100 bg-white/70 backdrop-blur-sm">
              ...
              <div className="flex space-x-2">
                ...
                {result && (
                  <CSVLink
                    data={csvData}
                    filename={`${resultTitle.toLowerCase().replace(/\s+/g, '-')}-${new Date().toISOString().slice(0, 10)}.csv`}
                    className="inline-flex items-center px-3 py-1.5 rounded-md bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition-colors ring-1 ring-indigo-200"
              ...
              {result.split('\n').map((line, index) => {
                ...
                if (line.match(/^#+\s/)) {
                  ...
                  if (level === 1)
                    return (
                      <h2
                        key={index}
                        className="text-xl font-bold text-gray-900 border-b pb-1 border-gray-200"
                      >
                        {content}
                      </h2>
                    );
                  ...
                  return (
                    <h4 key={index} className="text-base font-medium text-gray-800 mt-3">
                      {content}
                    </h4>
                  );
                }
                ...
                if (line.trim()) {
                  ...
                  const formattedText = line.replace(
                    /`(.*?)`/g,
                    '<span class="px-1 bg-indigo-50 text-indigo-700 rounded font-mono text-sm">$1</span>'
                  );
