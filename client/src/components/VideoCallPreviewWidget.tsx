...
const VideoCallPreviewWidget = React.memo(() => {
  ...
  const { currentCall, callStatus, initiateCall, initiateGroupCall, isGroupCall, participants } =
    useVideoCall();
  ...
  if (isGroupCallSetup) {
    return (
      <div className="fixed bottom-6 right-6 z-40">
        <div
          className={`${isDark ? 'bg-gray-900/95' : 'bg-white/95'} backdrop-blur-2xl border ${isDark ? 'border-white/20' : 'border-gray-200'} rounded-2xl overflow-hidden shadow-2xl w-96 max-h-[80vh] flex flex-col`}
        >
          {/* Header */}
          <div
  ...
  if (isExpanded) {
    return (
      <div className="fixed bottom-6 right-6 z-40">
        <div
          className={`${isDark ? 'bg-gray-900/95' : 'bg-white/95'} backdrop-blur-2xl border ${isDark ? 'border-white/20' : 'border-gray-200'} rounded-2xl overflow-hidden shadow-2xl w-80 max-h-96`}
        >
          {/* Header */}
          <div
            className={`p-4 border-b ${isDark ? 'border-white/10' : 'border-gray-200'} flex items-center justify-between`}
          ...
          {/* Quick Actions */}
          <div
            className={`p-4 border-t ${isDark ? 'border-white/10' : 'border-gray-200'} bg-gradient-to-r ${isDark ? 'from-gray-800/50 to-gray-700/50' : 'from-gray-50 to-gray-100'}`}
          >
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                className={`flex items-center justify-center space-x-2 p-3 rounded-lg ${isDark ? 'bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-400' : 'bg-indigo-100 hover:bg-indigo-200 text-indigo-700'} transition-colors`}
              >
