...
interface CommunicationStore {
  ...
  createSequence: (
    sequence: Omit<
      EmailSequence,
      'id' | 'createdAt' | 'updatedAt' | 'contactCount' | 'completionRate'
    >
  ) => void;
  ...
  persist(
    (set, get) => ({
      ...
      updateEmail: (id, updates) => {
        set((state) => ({
          emails: state.emails.map((email) =>
            email.id === id ? { ...email, ...updates, updatedAt: new Date() } : email
          ),
        }));
      },
      ...
      updateCallLog: (id, updates) => {
        set((state) => ({
          callLogs: state.callLogs.map((call) => (call.id === id ? { ...call, ...updates } : call)),
        }));
      },
      ...
      getEmailAnalytics: (dateRange) => {
        ...
        if (dateRange) {
          filteredEmails = filteredEmails.filter(
            (email) => email.createdAt >= dateRange.start && email.createdAt <= dateRange.end
          );
        }
      ...
      getFilteredEmails: () => {
        ...
        if (filter.dateRange) {
          filtered = filtered.filter(
            (email) =>
              email.createdAt >= filter.dateRange!.start && email.createdAt <= filter.dateRange!.end
          );
        }
