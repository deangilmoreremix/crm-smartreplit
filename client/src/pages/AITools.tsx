...
const AITools: React.FC = () => {
  ...
  const aiFeatures = [
    ...
    // Advanced AI features
    {
      title: 'AI Assistant',
      description:
        'Interact with a persistent AI assistant that remembers context and can help with various sales tasks',
    ...
    },
    {
      ...
      description:
        'Analyze images, screenshots, and visual content to extract insights and information',
    ...
    },
    {
      ...
      description:
        'Create branded marketing materials with SmartCRM templates, history tracking, and drag-and-drop seed images',
    ...
    },
    {
      ...
      description:
        'Find anything in your CRM with natural language queries and contextual understanding',
    ...
    },
    {
      ...
      description: 'Create and manage automated AI workflows',
      icon: <Zap className="h-6 w-6 text-yellow-600" />,
      id: 'agent-workflow' as const,
      categories: ['advanced', 'automation'],
      new: true,
      isPremium: true,
    },
    ...
    },
    {
      ...
      description:
        'Chat with an AI that can perform real actions in your CRM through natural conversation',
      icon: <Zap className="h-6 w-6 text-yellow-600" />,
  ...
  const filteredTools = aiFeatures.filter((tool) => {
    ...
    const matchesCategory =
      !activeCategory || activeCategory === 'all' || tool.categories.includes(activeCategory);
  ...
  // Function to render individual AI tool components based on currentTool
  const renderAITool = (toolId: string) => {
    switch (toolId) {
      ...
      case 'realtime-email-composer':
        return <RealTimeEmailComposer />;
      case 'form-validation':
        return <RealTimeFormValidation />;
        ...
        <div className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 p-8 rounded-xl shadow-sm mb-10 border border-blue-100">
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-bold mb-3 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600">
                Complete AI Tools Suite
              </h2>
              <p className="text-gray-700 text-lg">
                Experience our latest AI capabilities powered by OpenAI's GPT-4o and other advanced
                models. Get intelligent responses, detailed analysis, and comprehensive results for
                all your CRM and sales needs.
            ...
            filteredTools.map((feature) => (
              ...
              >
                <div className="flex flex-col h-full">
                  <div
                    className="mb-4 p-4 rounded-2xl bg-gradient-to-r transition-all duration-300 group-hover:scale-105 group-hover:shadow-sm relative"
                    style={{
                      backgroundImage: `linear-gradient(to right, ${getGradientColors(feature.id)})`,
                    }}
                  >
                    {feature.icon}
                    {feature.new && (
                      <span className="absolute top-0 right-0 -mt-2 -mr-2 bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full">
                        NEW
                      </span>
                    )}
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gradient-to-r from-indigo-50 to-blue-50">
                <h3 className="font-bold text-lg flex items-center">
                  ...
                  {activeDemoTool === 'streaming-chat'
                    ? 'Real-time Streaming Chat'
                    : activeDemoTool === 'form-validation'
                      ? 'Real-time Form Validation'
                      : activeDemoTool === 'live-deal-analysis'
                        ? 'Live Deal Analysis'
                        : activeDemoTool === 'instant-response'
                          ? 'Instant Response Generator'
                          : activeDemoTool === 'document-analyzer'
                            ? 'Real-time Document Analyzer'
                            : activeDemoTool === 'realtime-email'
                              ? 'Real-time Email Composer'
                              : activeDemoTool === 'voice-analysis'
                                ? 'Real-time Voice Analysis'
                                : activeDemoTool === 'smart-search'
                                  ? 'Smart Search with Typeahead'
                                  : activeDemoTool === 'auto-form'
                                    ? 'AI Form Auto-completion'
                                    : activeDemoTool === 'agent-workflow'
                                      ? 'Agent Workflow Chat'
                                      : 'Interactive Demo'}
...
const getGradientColors = (id: string): string => {
  switch (id) {
    ...
    case 'agent-workflow':
      return 'rgba(253, 186, 116, 0.1), rgba(253, 186, 116, 0.2)';
    default:
