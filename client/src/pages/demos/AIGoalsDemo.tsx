...
export default function AIGoalsDemo() {
  const goals = [
    ...
    {
      id: 4,
      title: 'Reduce Sales Cycle by 15 Days',
      progress: 92,
      status: 'completed',
      category: 'Efficiency',
    },
            ...
            {goals.map((goal) => (
              <div key={goal.id} className="p-4 border rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-sm ${goal.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}
                  >
                    {goal.status}
                  </span>
                </div>
