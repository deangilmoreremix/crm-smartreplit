...
const VideoEmail: React.FC = () => {
  ...
  const generateTalkingPoints = async () => {
    ...
    try {
      ...
      const newTalkingPoints: TalkingPoint[] = mockTalkingPoints.map(
              ...
              {recordings.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 mb-4">
                    Start recording to create your first video email
                  </p>
                  <button
                    onClick={() => setActiveTab('record')}
            ...
            {talkingPoints.length === 0 ? (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-gray-500">
                  Generate AI-powered talking points for your video
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                {talkingPoints.map((point) => (
                  <div key={point.id} className="flex items-start space-x-2">
                    ...
                    />
                    <span
                      className={`text-sm ${point.completed ? 'line-through text-gray-500' : 'text-gray-700'}`}
                    >
                      {point.text}
                    </span>
                  </div>
