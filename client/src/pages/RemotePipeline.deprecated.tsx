// DEPRECATED: This file is abandoned. Use PipelinePage.tsx instead.
  ...
  useEffect(() => {
    ...
    if (iframe && bridgeRef.current) {
      const handleLoad = () => {
        ...
        try {
          ...
          if (iframeDoc && iframeDoc.title === 'Page not found') {
            ...
            setError(
              'The remote React app is not available at this URL. Please check if the app is properly deployed.'
            );
            setIsLoading(false);
      ...
      const handleError = () => {
        ...
        setError(
          'Network error: Could not connect to the remote pipeline application. Check your internet connection.'
        );
        setIsLoading(false);
        setIsConnected(false);
      };
    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white border-t border-gray-200 text-xs text-gray-500">
        <div className="flex items-center space-x-4">
          ...
          {isConnected && <span className="text-green-600">● Bridge Active</span>}
          {error && <span className="text-red-600">● Connection Failed</span>}
        </div>
        <div className="flex items-center space-x-2">
