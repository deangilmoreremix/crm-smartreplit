 ...
 */
async function getOpenClawConfigForUser(
  userId: string
...
router.post('/chat', async (req, res) => {
  try {
    ...
    const { apiKey: effectiveApiKey, baseUrl: effectiveBaseUrl } =
      await getOpenClawConfigForUser(userId);
  ...
  try {
    switch (toolName) {
      ...
      case 'enrich_contact': {
        ...
        if (!contact) {
          memoryService
            .recordObservation(userId, 'error', `enrich_contact: Contact ${contactId} not found`, {
              tool: 'enrich_contact',
              contactId,
            })
            .catch(() => {});
        ...
        if (!client) {
          memoryService
            .recordObservation(userId, 'system_event', 'enrich_contact: OpenAI not configured', {
              tool: 'enrich_contact',
              contactId,
            })
            .catch(() => {});
      ...
      case 'analyze_sentiment': {
        ...
        if (!text) {
          memoryService
            .recordObservation(userId, 'error', 'analyze_sentiment: missing text parameter', {
        ...
        if (!client) {
          memoryService
            .recordObservation(userId, 'system_event', 'analyze_sentiment: OpenAI not configured', {
              tool: 'analyze_sentiment',
            })
            .catch(() => {});
          return { error: 'OpenAI API key not configured' };
      ...
      case 'bulk_analyze_contacts': {
        ...
        if (contactIds.length === 0) {
          memoryService
            .recordObservation(userId, 'error', 'bulk_analyze_contacts: no valid contact IDs', {
        ...
        if (contactIds.length > 50) {
          memoryService
            .recordObservation(userId, 'error', 'bulk_analyze_contacts: exceeded limit', {
              tool: 'bulk_analyze_contacts',
        ...
        memoryService
          .recordObservation(
            ...
            `Bulk analysis completed: ${successCount} succeeded, ${errorCount} failed out of ${contactIds.length} contacts`,
            {
              ...
              successCount,
              errorCount,
              errorRate: successCount / contactIds.length,
            }
          )
          .catch(() => {});
        ...
        return {
          results,
          summary: { total: contactIds.length, success: successCount, failed: errorCount },
        };
      }
      ...
      case 'analyze_deal': {
        ...
        if (!deal) {
          memoryService
            .recordObservation(userId, 'error', `analyze_deal: Deal ${dealId} not found`, {
              tool: 'analyze_deal',
              dealId,
            })
            .catch(() => {});
        ...
        if (!client) {
          memoryService
            .recordObservation(userId, 'system_event', 'analyze_deal: OpenAI not configured', {
              tool: 'analyze_deal',
              dealId,
              dealName: deal.name,
            })
