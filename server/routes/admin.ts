...
export function registerAdminRoutes(app: Express): void {
  ...
  app.patch(
    '/api/users/:id/product-tier',
    adminRateLimit,
    requireAdmin,
    sanitizeInput,
    validateUserData(['productTier']),
    auditTierChange,
    async (req: any, res) => {
      try {
        const { id } = req.params;
  ...
  app.post(
    '/api/admin/send-bulk-password-setup',
    adminRateLimit,
    requireAdmin,
    auditEmailSend,
    async (req: any, res) => {
  ...
  app.post(
    '/api/admin/bulk-actions/suspend-inactive',
    adminRateLimit,
    requireAdmin,
    auditUserUpdate,
    async (req: any, res) => {
  ...
  app.post(
    '/api/admin/bulk-actions/upgrade-trials',
    adminRateLimit,
    requireAdmin,
    auditUserUpdate,
    async (req: any, res) => {
      try {
        ...
        const trialUsers = await db
          ...
          .from(profiles)
          .where(
            sql`${profiles.productTier} IN ('smartcrm', 'sales_maximizer') AND ${profiles.role} != 'super_admin'`
          );
    ...
    try {
      ...
      if (dateFrom) {
        filteredLogs = filteredLogs.filter(
          (log) => new Date(log.createdAt) >= new Date(dateFrom as string)
        );
      }
      if (dateTo) {
        filteredLogs = filteredLogs.filter(
          (log) => new Date(log.createdAt) <= new Date(dateTo as string)
        );
      }
  ...
  app.post(
    '/api/admin/automated-actions/setup',
    adminRateLimit,
    requireAdmin,
    async (req: any, res) => {
      try {
      ...
      try {
        ...
        for (const userData of users) {
          try {
            ...
            errors.push(
              `Failed to import ${userData.email}: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
            failed++;
          }
        }
