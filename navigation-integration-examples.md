...
function onContactCreated(contact) {
  ...
  window.remoteBridge.showNotification(`Contact ${contact.name} created successfully!`, 'success');
}
...
function CRMNavigationComponent() {
  ...
  return (
    <div className="crm-navigation">
      ...
      <button onClick={() => handleNavigate('/contacts')}>Contacts</button>
      <button onClick={() => handleNavigate('/deals')}>Deals</button>
    </div>
  );
}
```
