...
var handler = async (event, context) => {
  ...
  try {
    ...
    if (pathParts.length >= 3 && pathParts[0] === "white-label" && pathParts[1] === "tenants" && httpMethod === "PUT") {
      ...
      const { data, error } = await supabase.from("tenants").update({ ...updates, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", tenantId).select().single();
      if (error) throw error;
      return { statusCode: 200, headers, body: JSON.stringify(data) };
    }
    ...
    if (pathParts.length >= 3 && pathParts[0] === "white-label" && pathParts[1] === "config" && httpMethod === "GET") {
      ...
      const { data, error } = await supabase.from("white_label_configs").select("*").eq("tenant_id", tenantId).single();
    ...
    if (pathParts.length >= 3 && pathParts[0] === "white-label" && pathParts[1] === "config" && httpMethod === "PUT") {
      ...
      const { data: existingConfig } = await supabase.from("white_label_configs").select("id").eq("tenant_id", tenantId).single();
      let result;
      if (existingConfig) {
        const { data, error } = await supabase.from("white_label_configs").update({ ...updates, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("tenant_id", tenantId).select().single();
        if (error) throw error;
        result = data;
      } else {
